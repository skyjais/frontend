...
const App: React.FC = () => {
  ...
  const handleCardFlip = (id: number) => {
    ...
    if (!selectedCard || flippedCards.includes(id) || flippedCards.length >= 2)
      return;
    ...
    if (flippedCards.length === 0 && selectedCard.color === "pink") {
      ...
      const firstCard = gameData?.cards.find(
        ...
        <>
          ...
          {matches === gameData.cards.length / 2 ? (
            ...
            <div className="parent">
              <div>
                <GameBoard
                  cards={gameData.cards.filter((card) => card.color === "pink")}
                  onCardFlip={handleCardFlip}
                  flippedCards={flippedCards}
                />
              </div>
              <GameBoard
                cards={gameData.cards.filter((card) => card.color === "blue")}
                onCardFlip={handleCardFlip}
                flippedCards={flippedCards}
              />
            </div>
          ...
          <div
            style={{ background: "rgba(204,217,26,255)", display: "inline" }}
          >
